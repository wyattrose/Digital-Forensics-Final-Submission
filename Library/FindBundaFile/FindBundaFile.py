...
class FindBundaFileIngestModuleFactory(IngestModuleFactoryAdapter):

    moduleName = "Bunda File Finder"

    def getModuleDisplayName(self):
        return self.moduleName

    def getModuleDescription(self):
        return "Module that finds Bunda.txt files"
    ...
    def createFileIngestModule(self, ingestOptions):
        return FindBundaFileIngestModule()


# File-level ingest module.  One gets created per thread.
class FindBundaFileIngestModule(FileIngestModule):

    _logger = Logger.getLogger(FindBundaFileIngestModuleFactory.moduleName)
    ...
    def process(self, file):
        ...
        if (file.getName() == 'Bunda.txt'):

            # Make an artifact on the blackboard.  TSK_INTERESTING_FILE_HIT is a generic type of
            # artifact.  Refer to the developer docs for other examples.
            art = file.newArtifact(BlackboardArtifact.ARTIFACT_TYPE.TSK_INTERESTING_FILE_HIT)
            att = BlackboardAttribute(BlackboardAttribute.ATTRIBUTE_TYPE.TSK_SET_NAME.getTypeID(), 
                  FindBundaFileIngestModuleFactory.moduleName, "Bunda File Finder")
            ...
            IngestServices.getInstance().fireModuleDataEvent(
                ModuleDataEvent(FindBundaFileIngestModuleFactory.moduleName, 
